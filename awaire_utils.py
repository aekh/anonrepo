...
class Requirement:
    ...
    def value_at(self, i):
        ...
        # if not(i >= self.n_values_forgotten):
        #     print("XYZ")
        #     print("hmm")
...
class Frontier:
    ...
    def try_expand_node(self, node, processed_nodes, force=False):
        ...
        if max(get_score(i) for i in sum(new_reqs.values(), [])) > max(self.node_expand_condition, node.score) or force:
            # if node.elimination_suffix[0] >= 5:
                # print(self.n_processed_ballots, node, node.score, node.value, max(get_score(i) for i in sum(new_reqs.values(), [])), [(i, get_score(i)) for i in node.watchlist])
            for child in new_reqs.keys():
                # if node.elimination_suffix[0] >= 5:
                #     print(" ~ ~", child, child.score, max(get_score(i) for i in new_reqs[child]), [(i, get_score(i)) for i in new_reqs[child]])
